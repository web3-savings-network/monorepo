...
export default async function deploy(hardhat: HardhatRuntimeEnvironment) {
  ...
  const svg = await deploy("svg", {
    contract: "@web3-savings-cards/core-sol/contracts/svg/svg.sol:svg",
    from: deployer,
    args: [],
  ...
  const svgUtils = await deploy("svgUtils", {
    contract: "@web3-savings-cards/core-sol/contracts/svg/svgUtils.sol:svgUtils",
    from: deployer,
    args: [],
    skipIfAlreadyDeployed: true,
    log: true,
