...
import { User, Submission } from './database'
...
// Data API
/* ------------------------------ */
app.post('/register', authenticateToken, async (req: any, res: any, next: Function) => {
  try {
    const { name, email, address: addressPassed } = req.body;
    const { address } = req?.user;
    if(ethers.utils.getAddress(address) != ethers.utils.getAddress(addressPassed)) {
...
app.post('/data/partner', authenticateToken, async (req: any, res: any, next: Function) => {
  try {
    const { name, email, address: addressPassed } = req.body;
    const { address } = req?.user;
    if(ethers.utils.getAddress(address) != ethers.utils.getAddress(addressPassed)) {
...
app.post('/data/institution', authenticateToken, async (req: any, res: any, next: Function) => {
  try {
    const { name, email, address: addressPassed } = req.body;
    const { address } = req?.user;
    if(ethers.utils.getAddress(address) != ethers.utils.getAddress(addressPassed)) {
